...
```
h' = alpha * h + (1-alpha) * A * h
```

其中：
- h：工具节点当前的向量表示（由 e5 模型编码工具描述得到）
- A：归一化的邻接矩阵，描述工具间的连接关系
- alpha：融合参数，控制自身信息与邻居信息的权重

alpha 取不同值时含义不同：

| alpha | 自身信息占比 | 邻居信息占比 | 含义 |
|-------|------------|------------|------|
| 1.0 | 100% | 0% | 完全不使用图信息（等于 No Graph） |
| 0.8 | 80% | 20% | 主要相信自身，少量参考邻居 |
| 0.5 | 50% | 50% | 对等地融合邻居信息 |

论文遍历了 11 个 alpha 值（0.5, 0.55, ..., 1.0）来寻找最优融合比例。
...
| 实验 | 内容 | 状态 |
|------|------|------|
| 实验 A | 评估 LLM Direct 预测结果 | [已完成] 已复现 |
| 实验 B | SGC 训练无关方法 | [已完成] 已复现 |
| 实验 C | LLM 直接推理（需部署 LLM） | [未完成] 使用作者预计算结果 |
| 实验 D | 训练 GNN 模型 | [未完成] 未复现 |
...
| --alpha_list | 默认 | 遍历 11 个 alpha 值 |

完整执行流程：

```
1. 加载 e5-large 模型 (335M 参数，约 2 秒)
2. 读取 23 个工具的描述文字 -> 用 e5 编码为 1024 维向量（约 1 秒）
3. 读取工具关系图 -> 构建邻接矩阵
4. SGC 图传播：h' = alpha*h + (1-alpha)*A*h（约 0.1 秒）
5. 读取 LLM 预计算结果
6. 对 497 个测试样本每个步骤：贪心选择最匹配的工具（约 20 秒）
7. 计算评估指标 -> 输出对比表格
总计约 25 秒（GPU 加速）
```
...
*图：运行 SGC 图增强实验的完整终端输出，包含 e5 模型加载、alpha 遍历结果表格*

**关键趋势分析：**

1. alpha=0.8 达到综合最优：Node-F1=0.658 (Direct 的 1.14 倍)，Link-F1=0.402 (Direct 的 1.39 倍)，Acc=0.235 (Direct 的 1.65 倍)
2. alpha 并非越大越好：alpha 从 0.5 增加到 0.8 时效果持续提升，但超过 0.8 后开始下降——说明适量的邻居信息（约 20%）有帮助，过多则引入噪声
3. No Graph 也优于 Direct：alpha=1.0（完全不使用图传播）的 Node-F1=0.634 > Direct 的 0.576，说明 e5-large 作为专用文本匹配模型，其编码质量高于 LLM 直接输出工具名

#### 四组 SGC 实验汇总

| 数据集 | LLM | Direct Acc | SGC 最佳 Acc | 提升幅度 |
|--------|-----|:---------:|:----------:|:--------:|
| HuggingFace | CodeLlama-13B | 0.1429 | 0.2354 (alpha=0.80) | +64.7% |
| Multimedia | CodeLlama-13B | 0.2410 | 0.3715 (alpha=0.85) | +54.1% |
| HuggingFace | Mistral-7B | 0.1636 | 0.2229 (alpha=0.80) | +36.2% |
| Multimedia | Mistral-7B | 0.2505 | 0.3368 (alpha=0.70) | +34.4% |
...
| 数据集 | LLM | 论文 SGC Node-F1 (最佳 alpha) | 本地 SGC Node-F1 (最佳 alpha) | 是否一致 |
|--------|-----|:----------:|:----------:|:--------:|
| HuggingFace | CodeLlama-13B | 65.51 | 0.6580 (alpha=0.80) | 基本一致 |

**偏差分析：**

本地 SGC 最佳 Node-F1 为 0.6580（alpha=0.80），论文报告值为 65.51（即 0.6551）。两者存在 0.0029 的微小差异。

可能原因：
1. alpha 取值不同：论文中最佳值 65.51 可能是 alpha=0.75 的结果（alpha=0.75 时本地 Node-F1=0.6591，四舍五入为 65.91，与论文值不符），也可能是论文采用了不同粒度的 alpha 搜索。本地在 alpha=0.70 时的结果为 0.6551，与论文报告的 65.51 完全一致。
...
| alpha=0.7-0.8 为最优区间 | 所有实验中最佳 alpha 均在 0.70-0.85 | 一致 |
...
因为 SGC 的贪心选择策略本质上是一个"约束搜索"——每次只能从上一个工具的邻居里选下一个工具。这相当于给 LLM 的输出加了一个图结构的合法性约束。LLM 直接推理时经常选择图上不存在的连接（幻觉率 LH-2 高达 63%），而 SGC 从根本上杜绝了这种可能。

**为什么 alpha=0.8 最优？**

alpha 控制"自己 vs 邻居"的信息权重。alpha 太小（0.5）意味着过于依赖邻居，丢失了工具自身的独特信息；alpha 太大（0.95-1.0）意味着几乎不使用图结构，退化为纯文本匹配。alpha=0.8 恰好平衡了两者——保留 80% 自身特征的同时融入 20% 的邻居上下文。这个比例在不同数据集上保持稳定（0.70-0.85），说明该方法具有一定的鲁棒性。

**为什么 No Graph 也比 Direct 好？**

No Graph（alpha=1.0）虽然放弃了图传播，但仍然使用 e5-large 做文本匹配。e5-large 是专门优化过的句子编码器，比 LLM（CodeLlama-13B）在"工具匹配"这个具体任务上更擅长。这说明：使用专用的小模型做特定任务，效果可能优于通用的大模型。这也是论文方法的一个额外启示。
